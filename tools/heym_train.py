#!/usr/bin/env python3
"""Train a real 'hey m' wake-word model on dataset/hey_m (2-class: wake vs not-wake).
Speaker-independent (heym_data held-out speakers). Reuses the AURA front-end, augmentation,
and model zoo. Exports .data/heym_<arch>.onnx + metrics. Run once per --arch for the
head-to-head."""
import argparse
import json
import math
...
def build(split, noises, aug_variants, seed):
    ...
    X, y = [], []
    for path, label, spk, acc in heym_data.items(split):
        x = heym_data.read_wav(path)
        X.append(fe.features(x, apply_dsp_chain=True, frames=fe.WINDOW_FRAMES))
        y.append(label)
        if split == "train":
            # Augment both classes; positives more (recall) and the HARD NEGATIVES
            # (confusables) most (FA-critical — generate confusable variety from our data).
    ...
    # and ambient are NOT the wake word" (fixes the 9% generic-speech false-fire). Only the
    # train split — the test set stays the held-out hey_m speakers for an honest FR/FA.
    if split == "train":
        sc = _sc_root()
        if sc:
            import sc_dataset
...
def main():
    ...
    cache = os.path.join(DATA, "heym_feat2.npz")  # expanded-negative cache (FA-reduction round)
    ...
    print(f"[{args.arch}] train={Xtr.shape} test={Xte.shape} pos_tr={int(ytr.sum())} "
          f"neg_tr={int((ytr==0).sum())} pos_te={int(yte.sum())}")
    ...
    # which would push false-accepts back up. FA reduction comes from the diverse negatives
    # + the Stage-2 cascade + operating point, not from biasing toward firing.
    ...
    onnx = os.path.join(DATA, f"heym_{args.arch}.onnx")
    try:
    ...
    json.dump(m, open(os.path.join(DATA, f"heym_{args.arch}_metrics.json"), "w"), indent=2)
    print(f"[{args.arch}] params={params} size={m['size_kb']}KB  speaker-indep F1={f1:.3f} "
