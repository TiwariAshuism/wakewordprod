...
record: what was measured on-host, and what is explicitly not yet verified. Detailed results
live in `HEYM_RESULTS.md` and `HEYM_FA_REDUCTION.md`.
...
## Front-end contract (frozen — `aura_sas.md` §19b)
...
- **Speaker-independent recall ≈ 97–98%** (per-clip 0.976–0.978; streaming-detectable ~97% by
  ...
  picking over-strict FA=0 points), **not** a model limitation — see `HEYM_FA_REDUCTION.md`.
...
- **Data is en-IN-dominant.** Live tags: en-IN 1430/331 (train/test) dwarf hi/ta/te/kn/ml-IN;
  **en-US, en-GB, en-AU are absent**. Cross-accent robustness for the V1 English accents is
  unmeasured. Mitigation: `MULTILINGUAL_PLAN.md`.
...
- **Speaker verification is spec'd, not shipped** (ADR-005 / `SPEAKER_VERIFICATION_PLAN.md`).
...
### References
- `HEYM_RESULTS.md` — winner selection, requirements table, M×threshold sweep.
- `HEYM_FA_REDUCTION.md` — cascade FA-reduction round and the FRR-artifact diagnostic.
- `DATASET_CARD.md`, `MULTILINGUAL_PLAN.md`, `SPEAKER_VERIFICATION_PLAN.md`, `DEVICE_RUNBOOK.md`.
