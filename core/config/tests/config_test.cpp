...
TEST(Config, DefaultsFullyPopulated) {
  ...
  EXPECT_TRUE(cfg->models.stage1WakeWord == "marvin");
}
...
TEST(Config, RuntimeOverridePublishesAndNotifies) {
  ...
  EXPECT_NEAR(before->detect.stage1Threshold, 0.5f, 1e-6);
  ...
  EXPECT_NEAR(before->detect.stage1Threshold, 0.5f, 1e-6);  // old snapshot unchanged
}
